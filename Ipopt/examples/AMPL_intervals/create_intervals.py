...
class AmplSet:
    ...
    def __init__(self, ampl_script, pLnames, pUnames, pLvalues, pUvalues,bm,sc,bv,ctrl_type,ctrl,algo,idx):
    ...
    def write_opt_file(self):
        ...
        f = open('ipopt.opt', 'w')
        f.write('#file generated by sIpOpt intervallization routine script create_intervals.py -- Ben Waldecker Sep 2012\n\n');
        options = self.create_optionstring(self.branchmode,self.scaling,self.benefits,self.algorithm,self.control,self.ctype)
        f.write(options)
        f.write('\n\n#end of file\n');
        f.close()

    def create_optionstring(self,bm,sc,bv,alg,ctrl,ctrlt):
        """
...
#        minBvalues = []
        for ir in range(nruns):
            output_f = open(self.outputname+str(ir)+'.txt', 'w')
            self.call_ampl(output_file_handle=output_f)
            output_f.close()
